...
const STATUS: Record<GameStatus, { label: string; chip: string }> = {
  ...
  "sold-out": {
    label: "Sold Out",
    chip: "border border-white/15 text-white/40",
  },
  broadcast: {
    label: "Live on TV",
    chip: "border border-ember-500/60 text-ember-400",
  },
};
      ...
      <div className="relative mx-auto w-full max-w-[1400px] px-5 sm:px-8 lg:px-14">
        ...
        >
          ...
          >
            ...
            <div
              aria-hidden="true"
              className="absolute inset-0 bg-linear-to-r from-ink-950 via-ink-950/90 to-ink-950/60"
            />
            <Grain />
            ...
            <div className="relative p-6 sm:p-10 lg:p-14">
              <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
                <span className="inline-flex items-center gap-3">
                  ...
                  <span className="font-display text-[0.68rem] font-bold uppercase tracking-[0.34em] text-ember-400">
                    Next Game
                  </span>
                </span>
                <span
                  className={`font-display clip-bastion-sm px-3 py-1 text-[0.62rem] font-bold uppercase tracking-[0.24em] ${
                    nextGame.home
                      ...
                      : "text-white/50 ring-1 ring-white/15"
                  }`}
                >
                ...
                <span className="font-display nums ml-auto text-sm font-bold uppercase tracking-[0.2em] text-white/60 sm:text-base">
                  {nextGame.day} {nextGame.date} {nextGame.month} ·{" "}
                  {nextGame.time}
                </span>
              </div>
              ...
              <h3 className="mt-8 grid grid-cols-1 items-center gap-4 text-center sm:mt-10 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] sm:gap-6 sm:text-left lg:gap-10">
                ...
                <span className="font-display relative block text-[clamp(1.6rem,5vw,2.75rem)] font-extrabold uppercase leading-none tracking-tight text-ember-500">
                  <span
                    aria-hidden="true"
                    className="mx-auto mb-3 block h-px w-16 rule-ember sm:hidden"
                  />
                  VS
                  <span
                    aria-hidden="true"
                    className="mx-auto mt-3 block h-px w-16 rule-ember sm:hidden"
                  />
                </span>
              ...
              <dl className="mt-9 grid grid-cols-1 divide-y divide-white/[0.08] border-t border-white/[0.08] sm:mt-12 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
                {[
                  ...
                  <div
                    key={item.term}
                    className="py-4 sm:px-6 sm:py-5 sm:first:pl-0"
                  >
                    <dt className="text-[0.62rem] font-semibold uppercase tracking-[0.34em] text-white/40">
                      {item.term}
                    </dt>
                    <dd className="font-display nums mt-2 text-base font-bold uppercase tracking-[0.05em] text-white/80 sm:text-lg">
                      {item.value}
                    </dd>
                  </div>
                ))}
              ...
              <div className="mt-9 sm:mt-12">
                <div role="timer" aria-live="off" aria-label={countdownLabel}>
                  <span className="text-[0.62rem] font-semibold uppercase tracking-[0.34em] text-white/40">
                  ...
                  {remaining?.done ? (
                    ...
                    <div
                      aria-hidden="true"
                      className="mt-3 grid grid-cols-4 gap-2 sm:gap-3 lg:max-w-[36rem]"
                    >
                      {cells.map((cell) => (
                        <div
                          key={cell.label}
                          className="clip-bastion-sm border border-white/10 bg-white/[0.03] px-1 py-4 text-center sm:py-5"
                        >
        ...
        <Reveal
          ...
          delay={80}
          distance={18}
          className="mt-16 flex items-end justify-between gap-6 sm:mt-20"
        >
          <h3 className="font-display text-sm font-bold uppercase tracking-[0.3em] text-white/70 sm:text-base">
            The run after
          </h3>
          <span className="font-display nums text-sm font-bold uppercase tracking-[0.24em] text-ember-500">
            {pad(fixtures.length)}
          </span>
        </Reveal>
        ...
        <ul className="mt-6 divide-y divide-white/[0.08] border-y border-white/[0.08]">
          {fixtures.map((game, index) => (
            ...
            >
              <a
                href="#contact"
                className="group relative grid grid-cols-[auto_minmax(0,1fr)] items-center gap-x-4 gap-y-3 px-3 py-5 sm:gap-x-6 sm:px-4 lg:grid-cols-[6.5rem_minmax(0,1fr)_minmax(0,14rem)_auto] lg:gap-x-8 lg:py-7"
                aria-label={`${game.day} ${game.date} ${game.month} — Black Fortress versus ${game.opponent}, ${game.venue}. ${STATUS[game.status].label}.`}
              >
                <span
                  aria-hidden="true"
                  className="absolute inset-0 bg-white/[0.03] opacity-0 transition-opacity duration-500 ease-[var(--ease-out-expo)] group-hover:opacity-100 group-focus-visible:opacity-100"
                />
                <span
                  aria-hidden="true"
                  className="absolute inset-x-0 bottom-0 h-px origin-left scale-x-0 rule-ember transition-transform duration-700 ease-[var(--ease-out-expo)] group-hover:scale-x-100 group-focus-visible:scale-x-100"
                />
                {game.home && (
                  <span
                    aria-hidden="true"
                    className="absolute inset-y-0 left-0 w-[2px] bg-linear-to-b from-transparent via-ember-500/55 to-transparent"
                  />
                )}

                <span className="relative block">
                  <span className="block text-[0.58rem] font-semibold uppercase tracking-[0.3em] text-white/40">
                    {game.day}
                  </span>
                  <span className="font-display nums mt-1 flex items-baseline gap-1.5 text-2xl font-extrabold uppercase leading-none tracking-tight text-white sm:text-3xl">
                    {game.date}
                    <span className="text-sm font-bold tracking-[0.1em] text-ember-500 sm:text-base">
                      {game.month}
                    </span>
                  </span>
                </span>
                ...
                <span className="relative col-span-2 flex items-center justify-between gap-3 lg:contents">
                  <span className="min-w-0 truncate text-xs text-white/45 sm:text-sm lg:text-white/50">
                    {game.venue}
                  </span>
                  <span
